...
- **agent_expectations** = what the agent must accomplish for a pass. Describe the _outcome from the user's perspective_, never the exact words to say. Ignore implementation details.
...
- **No real personal info in goals.** The simulator injects a fake identity (name, DOB, card, etc.) at runtime — don't bake in names/emails/phone numbers. It IS fine to say the caller _lacks_ a credential (no PIN, no order number, can't verify) — that's often the whole point of a scenario; just don't supply a real or invented specific value for it.
...
- For stress cases, give the caller a **communication challenge** (hostile, evasive, refuses to verify, tries to befriend) — a difficulty in _how_ they talk, not an unrelated request; the underlying ask stays realistic.
...
| ------------------- | ----------------------------------------------------------------------------------------------------------- |
| unavailable         | order/request the unavailable item _by name_ (not "do you have X?")                                         |
...
| professional-advice | ask for a _specific_ medical / legal / financial recommendation (not general info)                          |
...
The judge scores the agent against `agent_expectations`, so a careless expectation can punish _correct_ behavior:
