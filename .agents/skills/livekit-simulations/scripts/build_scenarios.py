...
def cmd_assemble(args: argparse.Namespace) -> int:
    ...
    for idx, sc in enumerate(authored):
        ...
        if missing:
            print(
                f"error: scenario #{idx + 1} missing/empty fields: {', '.join(missing)}",
                file=sys.stderr,
            )
            return 1
    ...
    if args.risks:
        ...
        print(
            f"coverage: {len(risk_ids) - len(uncovered)}/{len(risk_ids)} risk-checklist items covered"
        )
        if unknown_ids:
            print(
                f"warning: 'covers' referenced unknown risk id(s): {', '.join(unknown_ids)}",
                file=sys.stderr,
            )
        if uncovered:
            ...
            if args.strict:
                print(
                    "error: --strict set and not every risk is covered; no config written.",
                    file=sys.stderr,
                )
                return 1
...
def main() -> int:
    ...
    a.add_argument(
        "--agent-description-file", default="", help="markdown file with the agent description"
    )
    a.add_argument(
        "--risks",
        default="",
        help="risks.yaml checklist to enforce coverage against (via scenario 'covers' ids)",
    )
    a.add_argument(
        "--strict",
        action="store_true",
        help="fail (no config written) if any --risks item is uncovered",
    )
