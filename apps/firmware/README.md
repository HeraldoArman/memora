...
requests a development LiveKit token, joins the room, and publishes camera + microphone
...
The firmware publishes OV3660 video as H.264 at 320x240/5 FPS and the onboard PDM
...
The dashboard token route creates the room token and dispatches the local backend worker.
The returned LiveKit `server_url` is then used by the ESP, so the ESP does not need the
LiveKit API secret. Start the local services with Docker Compose and `bun run dev`, and
allow TCP port 3000 through the host firewall. The dashboard dev server is configured to
listen on `0.0.0.0` for this LAN access.
